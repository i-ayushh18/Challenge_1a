...
NOISE_PATTERNS = [
    ...
    r'^\d{1,2}-\d{1,2}-\d{2,4}$',  # More date formats
    r'^\d{1,2}\.\d{1,2}\.\d{2,4}$',  # Dotted date
    r'^\d{1,2} [A-Za-z]+ \d{4}$',  # 19 July 2023
    r'^\d{1,2}\.$',  # Just a number with dot (list item)
    r'^\d+$',  # Just a number
    r'^\d+\.$',  # Number with dot
    r'^\d+\)$',  # Number with parenthesis
    r'^\s*$',  # Empty
...
def is_date_like(text):
    """Return True if text looks like a date or date range (e.g., 'March 21, 2003', 'April 2004 – December 2006')"""
    text = text.strip()
    # Month names (full and abbreviated)
    full_months = [m for m in calendar.month_name[1:]]
    abbr_months = [m[:3] for m in calendar.month_name[1:]]
    month_names = '|'.join(full_months + abbr_months)
    # e.g., March 21, 2003 or Mar 21, 2003
    if re.match(rf'^(?:{month_names}) \d{{1,2}},? \d{{4}}$', text, re.IGNORECASE):
        return True
    # e.g., March 2003 or Mar 2003
    if re.match(rf'^(?:{month_names}) \d{{4}}$', text, re.IGNORECASE):
        return True
    # e.g., April 2004 – December 2006
    if re.match(rf'^(?:{month_names}) \d{{4}} ?[–-] ?(?:{month_names}) \d{{4}}$', text, re.IGNORECASE):
        return True
    # e.g., 2003 – 2006
    if re.match(r'^\d{4} ?[–-] ?\d{4}$', text):
        return True
    # e.g., April 11
    if re.match(rf'^(?:{month_names}) \d{{1,2}}$', text, re.IGNORECASE):
        return True
    # e.g., January 2007 -
    if re.match(rf'^(?:{month_names}) \d{{4}} ?[–-]?$', text, re.IGNORECASE):
        return True
    # e.g., 21 March 2003
    if re.match(rf'^\d{{1,2}} (?:{month_names}) \d{{4}}$', text, re.IGNORECASE):
        return True
    # e.g., 21/03/2003, 21-03-2003
...
def is_noise_heading(text):
    ...
    for pattern in NOISE_PATTERNS:
        if re.match(pattern, text):
            return True
    # Additional: skip if text is a date or just a number
...
class EnsembleHeadingDetector:
    ...
    def train_ml_model(self, training_data: List[Tuple[List[SpanInfo], List[bool]]]):
        """Train ML model to learn optimal weights"""
        all_features = []
        all_labels = []
        
        for spans, labels in training_data:
        ...
        # Update signal weights with learned coefficients
        coefficients = self.ml_model.coef_[0]
        total_weight = np.sum(np.abs(coefficients))
        
        for i, signal in enumerate(self.signals):
            signal.weight = abs(coefficients[i]) / total_weight
            print(f"{signal.name}: {signal.weight:.3f}")
        
        self.is_trained = True
    
    def detect_headings(self, spans: List[SpanInfo]) -> List[HeadingCandidate]:
        """Detect headings using ensemble approach"""
        if not spans:
            return []
        ...
        # Sort by confidence and refine levels
        candidates.sort(key=lambda x: x.confidence, reverse=True)
        return self._refine_heading_levels(candidates)
    
    def _merge_consecutive_headings(self, candidates):
        """Merge consecutive heading candidates on the same page and line."""
        if not candidates:
            return []
        merged = []
        prev = None
        for c in candidates:
            if prev and c.page == prev.page and abs(c.bbox[1] - prev.bbox[1]) < 5:  # Same line (y-coord)
                # Merge text
                prev.text += ' ' + c.text
    ...
    def get_document_outline(self, spans):
        """
        Get document title and outline (filtered, deduplicated, merged, and structured).
        """
        import logging
        ...
        # Merge consecutive heading spans
        ...
        for c in merged_candidates:
            ...
            if key not in seen and not is_noise_heading(c.text):
                # Skip headings that are just numbers or dates
                if re.match(r'^\d+[\.)]?$', c.text.strip()):
                    continue
                if is_date_like(c.text.strip()):
                    continue
                unique_candidates.append(c)
                seen.add(key)
        # Improved title extraction: largest, boldest, non-noise, non-date, non-number text on first page
        first_page_spans = [s for s in spans if s.page == 1 and not is_noise_heading(s.text)]
        # Filter out spans that are just numbers or dates
        first_page_spans = [s for s in first_page_spans if not re.match(r'^\d+[\.)]?$', s.text.strip()) and not is_date_like(s.text.strip())]
        # Further filter for valid titles
        title_candidates = [s for s in first_page_spans if is_valid_title(s.text)]
        if title_candidates:
            title_span = max(title_candidates, key=lambda s: (s.size, s.is_bold, len(s.text)))
            title = title_span.text
        else:
            title = "Untitled Document"
        # Build outline (only H1/H2/H3, output as required)
        outline = []
        for candidate in unique_candidates:
            if candidate.level in ['H1', 'H2', 'H3']:
                outline.append({
                    ...
                    'text': candidate.text,
                    'page': candidate.page - 1,  # Convert to 0-based if needed
                    'bbox': candidate.bbox,
                    'metadata': candidate.context
                })
        return title, outline

    def extract_refined_texts(self, spans, outline):
        """
        For each heading in outline, extract the text between this heading and the next heading (or end of document).
        Returns a list of dicts: { 'heading': ..., 'refined_text': ... }
        """
        results = []
        # Group spans by page
        spans_by_page = {}
        ...
        for i, heading in enumerate(outline):
            ...
            if i+1 < len(outline) and outline[i+1]['page'] == heading['page']:
                next_heading_text = outline[i+1]['text']
    ...
    def _determine_level(self, confidence: float, span: SpanInfo, spans: List[SpanInfo]) -> str:
        """Determine heading level based on confidence and context"""
    ...
    def _build_candidate_context(self, span: SpanInfo, spans: List[SpanInfo]) -> Dict[str, Any]:
        """Build context information for a candidate"""
        return {
            'font_size': span.size,
...
class RealTimeOptimization:
    ...
    def optimize_during_processing(self, detector: 'EnhancedEnsembleHeadingDetector', 
        ...
        # Adjust thresholds based on batch characteristics
        if batch_stats['avg_confidence'] < 0.5:
            # Lower thresholds for difficult documents
        ...
        if batch_stats['font_size_variance'] < 2:
            ...
            for signal in detector.signals:
                if signal.name == 'FontSizeSignal':
                    ...
                    signal.weight *= 1.2  # Increase position weight
...
class ExplainableAI:
    ...
    def _calculate_feature_contributions(self, candidate: HeadingCandidate) -> Dict[str, float]:
        ...
        for signal_name, score in candidate.signals.items():
            # Simple contribution calculation (in real implementation, use SHAP or LIME)
            contributions[signal_name] = score * 0.2  # Normalized contribution
    ...
    def _generate_alternatives(self, candidate: HeadingCandidate) -> List[Dict[str, Any]]:
        ...
        # What if confidence was different?
        if candidate.confidence >= 0.8:
            alternatives.append({
        ...
        # What if key signals were different?
...
FIELD_LABELS = {"to", "from", "date", "form", "name", "address", "subject", "reference"}

def is_valid_title(text):
    text = text.strip()
    # Skip field labels
    if text.lower() in FIELD_LABELS:
        return False
