...
class DocumentOutline:
    ...
    def to_dict(self) -> Dict[str, Any]:
        """Convert to dictionary for schema-compliant serialization"""
        return {
            'title': self.title,
            'outline': [
                {
                    'level': f"H{h.level}" if not str(h.level).startswith('H') else str(h.level),
                    'text': h.text,
                    'page': h.page
                }
...
class PDFOutlineBuilder:
    ...
    def _generate_cache_key(self, pdf_path: str) -> str:
        ...
        # Including file modification time and size in key
        import os
        try:
...
def demo_usage():
    ...
    # 2. Enhanced usage with custom configuration
    print("\n2. Enhanced Usage:")
    config = OutlineBuilderConfig()
    ...
    # outline = enhanced_builder.build_outline("example.pdf")
